...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'create') {
		...
		$stmt = $pdo->prepare('INSERT INTO grounds (name, location, description, price_per_hour, admin_id, is_active) VALUES (?, ?, ?, ?, ?, ?)');
		$stmt->execute([
			trim($_POST['name'] ?? ''),
			trim($_POST['location'] ?? ''),
	...
	if ($action === 'update') {
		...
		$stmt = $pdo->prepare('UPDATE grounds SET name=?, location=?, description=?, price_per_hour=?, admin_id=?, is_active=? WHERE id=?');
		$stmt->execute([
			trim($_POST['name'] ?? ''),
			trim($_POST['location'] ?? ''),
			trim($_POST['description'] ?? ''),
...
?>
	...
	<div class="grid">
		...
		<div class="card">
			<h3>All Grounds</h3>
			<table>
				<thead><tr><th>Name</th><th>Location</th><th>Admin</th><th>Price</th><th>Active</th><th>Actions</th></tr></thead>
				<tbody>
