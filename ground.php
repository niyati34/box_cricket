		...
		<div class="card mb-8 text-center">
			<div class="w-full h-64 bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-lg flex items-center justify-center mb-6">
				<?php if (!empty($ground['image_path'])): ?>
					<img src="<?php
						$img = $ground['image_path'];
						if (strpos($img, 'http') === 0) {
							echo htmlspecialchars($img);
						} else {
							echo BASE_URL . '/uploads/grounds/' . basename($img);
						}
					?>" alt="<?php echo htmlspecialchars($ground['name']); ?>" style="width:100%;height:100%;object-fit:cover;border-radius:10px;" />
				<?php else: ?>
			...
			<?php if (!empty($ground['maps_link'])): ?>
				<div class="mt-6">
					<a href="<?php echo htmlspecialchars($ground['maps_link']); ?>" class="btn secondary inline-flex items-center" target="_blank" rel="noopener">
						🗺️ View Map
					</a>
				</div>
			<?php endif; ?>
